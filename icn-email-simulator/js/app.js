...
// ---------------------------------------------------------------------------
// Les taux de clic réels tournent souvent autour de 0,1-2 % : un arrondi à
// l'entier les affiche comme "0%", peu informatif. On garde 1-2 décimales
// en dessous de 5 %, l'arrondi entier au-delà (où il reste lisible).
function pct(v) {
  const p = v * 100;
  if (p === 0) return '0%';
...
function benchmarkNote(rate, [low, high]) {
  const p = rate * 100;
  if (p < low) return { text: `En dessous du repère ICN (${low}-${high}%)`, cls: 'bad' };
  if (p > high) return { text: `Au dessus du repère ICN (${low}-${high}%)`, cls: 'good' };
  return { text: `Dans le repère ICN (${low}-${high}%)`, cls: 'ok' };
}
...
function renderDashboard({ email, profiles, result, appeal, recs, segmentIds, statutIds }) {
  ...
  // Repères calés sur 6 campagnes ICN réelles (cf. data/calibration-campaigns.mjs),
  // pas des benchmarks sectoriels génériques : les envois froids à des leads
  // (Parcoursup, nurture) performent nettement en-dessous des benchmarks
  // "newsletter à audience engagée" habituellement cités.
  const openBench = benchmarkNote(result.openRate, [10, 30]);
  const clickBench = benchmarkNote(result.clickRate, [0.2, 1.5]);
