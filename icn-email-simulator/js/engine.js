...
const THEME_KEYWORDS = {
  ...
  urgence: [
    ...
    // Comptes à rebours relatifs ("dans 2 jours") — corrélés à un net gain
    // de taux d'ouverture sur les campagnes ICN réelles utilisées pour la
    // calibration (webinaires annoncés à J-1/J-2 vs. contenus evergreen).
    'dans 1 jour', 'dans 2 jours', 'dans 3 jours', 'dans 4 jours', 'dans 5 jours',
    'dans quelques jours', "c'est demain", 'demain à', "c'est dans",
  ],
};
...
export function analyzeEmail(email) {
  ...
  return {
    ...
    meta: {
      ...
      // Proxy simple pour "email annonçant un événement daté" (webinaire,
      // JPO...) : un jour de semaine + une heure quelque part dans le texte.
      mentionsScheduledEvent: /lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche/i.test(fullText) && /\d{1,2}\s?h(\d{2})?\b/i.test(fullText),
    },
  };
}
...
export function logistic(x, midpoint, steepness) {
  return 1 / (1 + Math.exp(-steepness * (x - midpoint)));
}

// Constantes de calibration — ajustées par régression sur des campagnes ICN
// réelles (cf. js/calibrate.mjs, data/calibration-campaigns.mjs et
// data/segment-targets.json, 405 campagnes / ~186k délivrés). Ne pas
// modifier à la main sans relancer la calibration : ces valeurs sont
// interdépendantes (steepness compense midpoint, les offsets par segment
// compensent la moyenne de population).
//
// segmentOffsetOpen/Click : décalage additif par segment appliqué au signal
// brut avant passage dans la logistique. Nécessaire car les cibles réelles
// n'ont PAS un engagement proportionnel au niveau académique — ex. Bac+4/5
// (CTOR réel 27,9%) engage bien plus que Terminale (CTOR réel 5,6%) alors
// que rien dans le contenu ne l'explique : c'est un effet de cible/canal
// (audience plus décidée, liste plus qualifiée) que le contenu seul ne
// capture pas.
export const CALIBRATION = {
  openMidpoint: 82,
  openSteepness: 0.049,
  clickMidpoint: 77,
  clickSteepness: 0.127,
  segmentOffsetOpen: { terminale: -12.6, bac1: -7.3, bac2: 9.2, bac3: -1.6, bac45: 6.4 },
  segmentOffsetClick: { terminale: -6.7, bac1: 5.3, bac2: 1.3, bac3: 5, bac45: 15.8 },
};

/**
 * Calcule les scores bruts (avant fonction logistique) d'ouverture et de
 * clic pour UN profil face à l'analyse d'un email. Séparé de scoreProfile
 * pour permettre la calibration (js/calibrate.mjs) de rejouer ces signaux
 * avec différents paramètres de logistique sans dupliquer la logique.
 */
export function computeRawSignals(profile, analysis) {
  ...
  // Urgence temporelle concrète dans l'objet/pré-header ("dans 2 jours") —
  // signal indépendant des centres d'intérêt du profil, mesuré comme un
  // gain net d'ouverture sur les campagnes réelles utilisées pour calibrer
  // le moteur (cf. data/calibration-campaigns.mjs).
  const urgencySignal = analysis.themes.subject.urgence * 0.32 + analysis.themes.preheader.urgence * 0.16;
  ...
  const openSignal =
    ...
    (CALIBRATION.segmentOffsetOpen[profile.segment] || 0);
  ...
  const clickSignal =
    ...
    (CALIBRATION.segmentOffsetClick[profile.segment] || 0);

  return { openSignal, clickSignal };
}
...
function scoreProfile(profile, analysis) {
  ...
  const openProbability = logistic(openSignal, CALIBRATION.openMidpoint, CALIBRATION.openSteepness);
  const clickGivenOpenProbability = logistic(clickSignal, CALIBRATION.clickMidpoint, CALIBRATION.clickSteepness);
...
export function buildRecommendations({ profiles, email, analysis, appeal, result, segmentLabels }) {
  ...
  if (analysis.meta.mentionsScheduledEvent && analysis.themes.subject.urgence < 15) {
    recs.push({ severity: 'suggestion', section: 'Objet', text: "Cet email annonce un événement daté (webinaire, JPO...) mais l'objet ne crée pas d'urgence concrète. Sur les campagnes ICN réelles, une formulation en compte à rebours (\"dans 2 jours\", \"demain à 18h\") est associée à un net gain de taux d'ouverture par rapport à un objet evergreen." });
  }
